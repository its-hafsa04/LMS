...
export const loginUser = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!email || !password) {
            return next(new ErrorHandler("Please enter both credentials.", 400))
        }
...
export const getUserInfo = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // If no tokens at all -> return null user (don't throw)
        ...
        // If access token invalid but refresh token exists, try refresh token
        if (refresh_token) {
            try {
                const decodedRef = jwt.verify(refresh_token, process.env.REFRESH_TOKEN as string) as JwtPayload;
                const session = await redis.get(decodedRef.id);
                if (!session) {
                    return res.status(200).json({ success: true, user: null });
                }
                const user = JSON.parse(session);
                // Return user (optionally re-issue tokens here if you want)
                return res.status(200).json({ success: true, user });
            } catch (err) {
                return res.status(200).json({ success: true, user: null });
            }
...
// update user avatar
interface IUpdatProfilePicture {
    avatar: string;
}
...
export const updateProfilePicture = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (avatar && user) {
            // if user have one avatarthen call this
            if (user?.avatar?.public_id) {
                // first edlete the old image
